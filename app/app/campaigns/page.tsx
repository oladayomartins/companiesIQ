...
async function loadStats(): Promise<CampaignStats | null> {
  ...
  try {
    ...
    const byEvent: Record<string, number> = { scan: 0, view: 0, lead: 0 };
    const bySource: Record<string, { scan: number; view: number; lead: number }> = {};
    for (const e of events ?? []) {
      const ev = (e.event as string) || "view";
      byEvent[ev] = (byEvent[ev] ?? 0) + 1;
      const src = (e.source as string) || "direct";
      bySource[src] = bySource[src] || { scan: 0, view: 0, lead: 0 };
