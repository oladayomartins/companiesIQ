...
export default async function MarketsPage() {
  ...
  let nomis = null as Awaited<ReturnType<typeof getRegionIndicators>> | null;
  let error: string | null = null;
  try {
    [kpis, trendData, activity, nomis] = await Promise.all([
      getRegisterKpis(30),
      getIncorporationTrend(),
    ...
    <div className="screen">
      <div className="screen-head">
        <div>
          ...
          <h1 className="screen-title">Regional &amp; industry intelligence</h1>
        </div>
      ...
      <div className="dash-cols" style={{ marginTop: 18 }}>
        ...
        <Card>
          <CardHeader subtitle="Live · last 30 days" title="Most active cities" />
          <RankList items={activity?.cities ?? []} />
        </Card>
      </div>

      <div className="dash-cols" style={{ marginTop: 18 }}>
        <Card>
          <CardHeader subtitle="Nomis regional index" title="Growth hotspots" action={<span className="app-eyebrow">Nomis</span>} />
          <CardBody flush>
            {hotspots.map((h, i) => (
