// PUBLIC industry intelligence — an indexable SEO landing page per sector.
// Same ONS/Nomis/Companies House engine as the gated app, presented for cold
// search traffic, linking out to individual /company reports and /market pages.
...
function statForSlug(slug: string) {
  return Object.values(SECTOR_STATS).find((s) => slugify(s.sector) === slug) ?? null;
}
...
export async function generateMetadata({ params }: { params: Promise<{ sector: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    description: desc,
    alternates: { canonical: `/industry/${sector}` },
    openGraph: { title: `${stat.sector} — UK industry data`, description: desc, url: `${SITE_URL}/industry/${sector}`, type: "website" },
  };
}
    ...
    <PublicShell>
      ...
      <div className="screen profile">
        ...
        <div className="profile-grid" style={{ marginTop: 18 }}>
          ...
          <Card>
            <CardHeader subtitle="Nomis" title="Where this sector grows" />
            <CardBody>
              <div className="sector-list">
                {regions.map((r) => (
                  <Link className="sector" key={r.region} href={`/market/${slugify(r.region)}`} style={{ textDecoration: "none" }}>
        ...
        <div style={{ marginTop: 18 }}>
          <Card>
            <CardHeader subtitle="Live · last 120 days" title="Recently registered in this sector" action={<Badge tone="pos" dot>Companies House</Badge>} />
            <CardBody flush>
